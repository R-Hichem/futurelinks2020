...
from futurelinks.helpers import intersection as myintersec
__all__ = [
    "AdamicAdar",
    "AssociationStrength",
    "CommonNeighbours",
...
class AdamicAdar(Predictor):
    def predict(self, weight=None):
        ...
        for a, b in self.likely_pairs():
            intersection = set(neighbourhood(self.G, a)) & set(
                neighbourhood(self.G, b))
            w = 0
...
class CommonNeighboursGF(Predictor):
    def predict(self, alpha=1.0, weight=None):
        r"""Predict using common neighbours
        ...
        for a, b in self.likely_pairs():
            if weight is None or alpha == 0.0:
                authorsArray = [a, b]
                common = myintersec(
                    list(self.G.neighbors(a)), list(self.G.neighbors(b))) + authorsArray
                # common_neighbours = set(neighbourhood(
                #     self.G, a, k=1)) & set(neighbourhood(self.G, b, k=1))
                # common_neighbours = common_neighbours
                subG = self.G.subgraph(common)
                cngfScore = 0
                for nodeID, nodeInfo in subG.nodes(data=True):
                    if nodeID not in authorsArray:
                        cngfScore = cngfScore + \
                            (subG.degree[nodeID] /
                             math.log10(self.G.degree[nodeID]))
                w = cngfScore
                # w = neighbourhood_intersection_size(self.G, a, b, weight=None)
            elif alpha == 1.0:
                common_neighbours = set(neighbourhood(
                    self.G, a, k=1)) & set(neighbourhood(self.G, b, k=1))
                common_neighbours = common_neighbours + [a, b]
                subG = self.G.subgraph(common_neighbours)
                cngfScore = 0
                for nodeID, nodeInfo in subG.nodes(data=True):
                    if nodeID not in common_neighbours:
                        cngfScore = cngfScore + \
                            (subG.degree[nodeID] /
                             math.log10(self.G.degree[nodeID]))
                w = cngfScore
    ...
    def predict(self, weight=None):
        ...
        for a, b in self.likely_pairs():
            ...
            for c in intersection:
                if weight is not None:
                    numerator = float(
                        self.G[a][c][weight] * self.G[b][c][weight])
                else:
                    numerator = 1
